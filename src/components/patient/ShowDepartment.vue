<template>
  ...
  <el-dialog v-model="centerDialogVisible" title="门诊预约提醒" width="60%" align-center
    ...
    <span>
      ...
      （2） 按照短信或挂号详情提示，在指定的就诊时问及就诊地点进行分诊、等待叫号，按序就诊。
      2.预约挂号
      （1） 可以预约未来7天的号源，当天15:00停止预约次日的号源。
    ...
    <span>
      四、取消预约及退号<br>
      （一）取消预约
      1.取消预约：如就诊当日不能前来就诊，请主动于就诊前一日15:00之前取消预约；
      2.接诊医师因疾病、紧急救治任务等原因停诊时，系统将自动取消预约，同时退还已支付费用。同时医院会对您的预约给予转诊处理，请注意保持电话畅通。
      3.超过15:00无法取消次日预约号，只能按退号处理，3个月内累计退号两次将进入黑名单，6个月内无法通过手机预约。
      （二）退号
      退号将不会退还医时服务费，请注意可以取消预约的日期或者按时就诊。
    </span><br>
    <span>
      五、注意事项<br>
